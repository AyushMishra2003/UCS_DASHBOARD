...
import { getDriverDetail, getGuiderDetail, getHotelDetail, getPriestDetail, updateDriverStatus, updateGuiderStatus, updateHotelStatus, updatePriestStatus } from '../../Redux/Slices/ListSlice';
...
const HotelDetail = () => {
    ...
    const hotelData = useSelector((state) => state?.list?.hotelDetail);
    const dispatch = useDispatch();
    const { id } = useParams();
    console.log(hotelData)
    const loadData = () => {
        setLoaderActive(true);
        dispatch(getHotelDetail(id)).finally(() => setLoaderActive(false));
    };
    ...
    const handleStatusChange = async (id, status) => {
        const res = await dispatch(updateHotelStatus({ id, status }))
            .unwrap()
            .then(() => {
    ...
    const otherFiles = hotelData?.proofFiles || [];
                ...
                <div className='flex items-center justify-center'>
                    <form className='p-6 bg-[#2F3349] text-white relative rounded-md shadow-[0px_0px_20px_#3D4056] overflow-hidden my-12 flex flex-col  gap-1 w-fit max-w-[100%] md:max-w-[80%] lg:max-w-[100%]'>
                        ...
                        <div className={mainDiv}>
                            ...
                            <input readOnly className={inputStyle} name='fullName' value={hotelData?.fullName || ''} id='fullName' type="text" />
                        </div>
                        <div className={mainDiv}>
                            <label className={labelStyle} htmlFor="email">Email</label>
                            <input readOnly className={inputStyle} type="email" name='email' value={hotelData?.email || ''} id='email' />
                        </div>
                        <div className={mainDiv}>
                            <label className={labelStyle} htmlFor="phoneNumber">Email</label>
                            <input readOnly className={inputStyle} type="number" name='phoneNumber' value={hotelData?.phoneNumber || ''} id='phoneNumber' />
                        </div>
                        <div className='flex justify-between gap-2'>
                            <div className={`${mainDiv} w-[48%]`}>
                                <label className={labelStyle} htmlFor="age">Age</label>
                                <input readOnly className={inputStyle} type="number" name='age' value={hotelData?.age || ''} id='age' />
                            </div>
                            <div className={`${mainDiv} w-[48%]`}>
                                <label className={labelStyle} htmlFor="experience">Experience</label>
                                <input readOnly className={inputStyle} type="number" name='experience' value={hotelData?.experience || ''} id='experience' />
                            </div>
                        </div>
                        ...
                        <div className='flex items-center gap-4 min-w-[6.8rem] justify-center bg-white shadow-[0px_0px_12px_#808080_inset] mt-1 p-2 rounded text-black'>
                            ...
                            <div className='flex items-center justify-center gap-[0.1rem]'>
                                ...
                                <input
                                    type='radio'
                                    name={`status`}
                                    checked={hotelData?.status === 'PENDING'}
                                    onChange={() => handleStatusChange(hotelData?._id, 'PENDING')}
                                    value='PENDING'
                            ...
                            <div className='flex items-center justify-center gap-[0.1rem]'>
                                ...
                                <input
                                    type='radio'
                                    name={`status`}
                                    checked={hotelData?.status === 'ACCEPTED'}
                                    onChange={() => handleStatusChange(hotelData?._id, 'ACCEPTED')}
                                    value='ACCEPTED'
                            ...
                            <div className='flex items-center justify-center gap-[0.1rem]'>
                                ...
                                <input
                                    type='radio'
                                    name={`status`}
                                    checked={hotelData?.status === 'REJECTED'}
                                    onChange={() => handleStatusChange(hotelData?._id, 'REJECTED')}
                                    value='REJECTED'
