...
import { getHotelList, updateHotelStatus } from '../../Redux/Slices/ListSlice';
...
const HotelsList = () => {
    ...
    const list = useSelector((state) => state?.list?.hotelList);
    ...
    const loadData = async (page = 1) => {
        ...
        try {
            ...
            const response = await dispatch(getHotelList(params)).unwrap();
    ...
    const handleStatusChange = async (id, status) => {
        try {
            await dispatch(updateHotelStatus({ id, status })).unwrap();
                    ...
                    {loading ? (
                        ...
                        list?.map((data, index) => (
                            <div key={data?._id} className='relative flex items-center justify-between w-full gap-3 px-3 py-3 text-black bg-white'>
                                ...
                                <div onClick={() => navigate(`/hotel/${data?._id}`, { state: data?._id })} className='min-w-[3.3rem] sticky px-5 right-0 bg-[white] flex items-center justify-center'>
