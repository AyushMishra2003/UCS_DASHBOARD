            ...
            <div className='flex flex-col items-center gap-4 mt-4 '>
                {loading ? (
                    ...
                    paginatedData.map((data) => (
                        <div
                            key={data?._id}
                            className={`flex cursor-pointer  lg:justify-between lg:min-w-[38rem] lg:flex-row overflow-hidden flex-col items-start md:w-[50vw] sm:w-[65vw] w-[90vw]  lg:w-[50vw] xl:w-[50rem] min-w-[18rem] rounded-lg shadow-md border-r-4 ${data.status === 'Completed' ? 'border-green-500' : data.status === 'Picked up' || data.status === 'On the way' ? 'border-orange-500' : data.status === 'Cancelled' ? 'border-red-500' : 'border-blue-500'}`}
                            onClick={(e) => {
