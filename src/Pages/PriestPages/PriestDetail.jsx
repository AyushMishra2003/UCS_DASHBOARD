...
import { getDriverDetail, getPriestDetail, updateDriverStatus, updatePriestStatus } from '../../Redux/Slices/ListSlice';
...
const PriestDetail = () => {
    ...
    const priestData = useSelector((state) => state?.list?.priestDetail);
    const dispatch = useDispatch();
    const { id } = useParams();

    const loadData = () => {
        setLoaderActive(true);
        dispatch(getPriestDetail(id)).finally(() => setLoaderActive(false));
    };
    ...
    const handleStatusChange = async (id, status) => {
        const res = await dispatch(updatePriestStatus({ id, status }))
            .unwrap()
            .then(() => {
    ...
    const otherFiles = priestData?.proofFiles || [];
            ...
            {loaderActive ? (
                ...
                <div className='flex items-center justify-center'>
                    <form className='p-6 bg-[#2F3349] text-white relative rounded-md shadow-[0px_0px_20px_#3D4056] overflow-hidden my-12 flex flex-col  gap-1 w-fit max-w-[100%] md:max-w-[80%] lg:max-w-[100%]'>
                        ...
                        <div className='mb-4'>
                            <h2 className='text-[1.8rem] font-semibold tracking-wide'>Pandit ji detail</h2>
                            <div className='flex items-center mt-2'>
                        ...
                        <div className={mainDiv}>
                            ...
                            <input readOnly className={inputStyle} name='fullName' value={priestData?.fullName || ''} id='fullName' type="text" />
                        </div>
                        <div className={mainDiv}>
                            <label className={labelStyle} htmlFor="email">Email</label>
                            <input readOnly className={inputStyle} type="email" name='email' value={priestData?.email || ''} id='email' />
                        </div>
                        <div className={mainDiv}>
                            <label className={labelStyle} htmlFor="phoneNumber">Email</label>
                            <input readOnly className={inputStyle} type="number" name='phoneNumber' value={priestData?.phoneNumber || ''} id='phoneNumber' />
                        </div>
                        <div className='flex justify-between gap-2'>
                            <div className={`${mainDiv} w-[48%]`}>
                                <label className={labelStyle} htmlFor="age">Age</label>
                                <input readOnly className={inputStyle} type="number" name='age' value={priestData?.age || ''} id='age' />
                            </div>
                            <div className={`${mainDiv} w-[48%]`}>
                                <label className={labelStyle} htmlFor="experience">Experience</label>
                                <input readOnly className={inputStyle} type="number" name='experience' value={priestData?.experience || ''} id='experience' />
                            </div>
                        </div>
                        ...
                        <div className='flex items-center gap-4 min-w-[6.8rem] justify-center bg-white shadow-[0px_0px_12px_#808080_inset] mt-1 p-2 rounded text-black'>
                            ...
                            <div className='flex items-center justify-center gap-[0.1rem]'>
                                ...
                                <input
                                    type='radio'
                                    name={`status`}
                                    checked={priestData?.status === 'PENDING'}
                                    onChange={() => handleStatusChange(priestData?._id, 'PENDING')}
                                    value='PENDING'
                            ...
                            <div className='flex items-center justify-center gap-[0.1rem]'>
                                ...
                                <input
                                    type='radio'
                                    name={`status`}
                                    checked={priestData?.status === 'ACCEPTED'}
                                    onChange={() => handleStatusChange(priestData?._id, 'ACCEPTED')}
                                    value='ACCEPTED'
                            ...
                            <div className='flex items-center justify-center gap-[0.1rem]'>
                                ...
                                <input
                                    type='radio'
                                    name={`status`}
                                    checked={priestData?.status === 'REJECTED'}
                                    onChange={() => handleStatusChange(priestData?._id, 'REJECTED')}
                                    value='REJECTED'
